...
// Структура для хранения позиции камеры
...
void Aircraft_turn_test(float H, float pitch, float yaw, float radius, float R_turn)
{

    // Настройка вершин
    float vert_x[] = {0, 0, H + R_turn / sin((90-pitch) * M_PI / 180), inter.x, inter.y, inter.z + R_turn / sin((90 - pitch) * M_PI / 180)};
    ...
    glBegin(GL_POINTS);
    glColor3d(0, 1, 0);
    glVertex3d(track.x, track.y, track.z); // Координаты точки начала ухода ЛА
    glEnd();
    ...
    glBegin(GL_POINTS);
    glColor3d(0.8, 1, 0.5);
    glVertex3d(contact.x, contact.y, contact.z); // Координаты точки начала ухода ЛА
    glEnd();
    ...
    glBegin(GL_POINTS);
    glColor3d(1, 0.8, 0);
    glVertex3d(turn.x, turn.y, turn.z); // Координаты центра радиуса окружности ухода ЛА
    glEnd();
    ...
    //cout << "=====================" << endl;
    for (int count_circle = 0; count_circle < 50; count_circle++)
    {
        if (count_circle > 19)
        {
    ...
    for (int count_sphere = 0; count_sphere < 50; count_sphere++)
    {
        ...
        {
            Draw_sphere_traectory(x_rotate, y_rotate, 0.05);
        }
    }
    // Возвращение матрицы из стека
    glPopMatrix();
 ...
 * @return Ничего
***********************************************************************************/
void Draw_sphere_(float x_sp, float y_sp, float r)
{
...
int main(void)
{
    ...
    */
    ...
    //angle_radius = Angle_two_vectors(2, 2, 0, 2, 2, 0);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (inter.flag_inter == true)
        {
            ...
            Axe();
            Cursors();
            Draw_sphere_(x_sphere, -y_sphere, radius_sphere);
            Show_plane();
            Aircraft_turn(H);
            //Track_aircraft(inter.x, inter.y, inter.z, H, V, pitch, yaw);
            bf_x = inter.x;
            bf_y = inter.y;
            bf_z = inter.z;
            ...
            Aircraft_turn_test(H, pitch, yaw, radius_sphere, R_turn);
            glPopMatrix();
        }
        else
        {
